function part3_miti_ML_census2020_clutter_rev11_mc_chunks(app,parallel_flag,rev_folder,workers,move_list_reliability,mc_size,mc_percentile,reliability,norm_aas_zero_elevation_data,string_prop_model,tf_opt,tf_server_status,tf_recalculate,array_mitigation,cell_aas_dist_data,cell_sim_data,move_list_margin)
        ...
        for folder_idx=1:1:num_folders
            ...
            if cell_status{temp_cell_idx,2}==0
                ...
                if var_exist==2
                    ...
                    disp_progress(app,strcat('Part3: Loading Data . . . '))
                    ...
                    custom_antenna_pattern=load_variable_with_retry_GPT_rev2(app, strcat(data_label1,'_custom_antenna_pattern.mat'),'custom_antenna_pattern');  % e.g. stop if it truly doesn't exist
                    %%%%[custom_antenna_pattern]=load_variable_with_retry(app, strcat(data_label1,'_custom_antenna_pattern.mat'),'custom_antenna_pattern');
                    CBSD_label='BaseStation';
                    neighborhood_radius=load_variable_with_retry_GPT_rev2(app, strcat(CBSD_label,'_',data_label1,'_catb_neighborhood_radius.mat'), "catb_neighborhood_radius");  % e.g. stop if it truly doesn't exist
                    disp_progress(app,strcat('neighborhood_radius:',num2str(neighborhood_radius)))
                    %pause;
                    ...
                    if file_union_move_exist==2
                        ...
                        % % % % % % % % % % % % % % % Parallel is not in the MC chunks
                        % % % % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Calculate first-->Parfor --> No data load
                        % % % % if parallel_flag==1
                        % % % %     [poolobj,cores]=start_parpool_poolsize_app(app,parallel_flag,workers);
                        % % % %     parfor point_idx=1:num_ppts  %%%%Change to parfor
                        % % % %         %pre_sort_movelist_rev21c_miti_superbsdist_app(app,move_list_reliability,point_idx,sim_number,mc_size,radar_beamwidth,base_protection_pts,min_ant_loss,radar_threshold,mc_percentile,sim_array_list_bs,data_label1,reliability,norm_aas_zero_elevation_data,string_prop_model,array_mitigation,tf_opt,min_azimuth,max_azimuth,neighborhood_radius,custom_antenna_pattern,bs_eirp_dist,cell_aas_dist_data);
                        % % % %         pre_sort_movelist_rev21d_miti_clutter_app(app,move_list_reliability,point_idx,sim_number,mc_size,radar_beamwidth,base_protection_pts,min_ant_loss,radar_threshold,mc_percentile,sim_array_list_bs,data_label1,reliability,norm_aas_zero_elevation_data,string_prop_model,array_mitigation,tf_opt,min_azimuth,max_azimuth,neighborhood_radius,custom_antenna_pattern,bs_eirp_dist,cell_aas_dist_data);
                        % % % %     end
                        % % % % end
                        ...
                        for point_idx=1:1:num_ppts  %%%%%%%%This can be parfor
                            point_idx
                            %[cell_miti_list_21d]=pre_sort_movelist_rev21d_miti_clutter_app(app,move_list_reliability,point_idx,sim_number,mc_size,radar_beamwidth,base_protection_pts,min_ant_loss,radar_threshold,mc_percentile,sim_array_list_bs,data_label1,reliability,norm_aas_zero_elevation_data,string_prop_model,array_mitigation,tf_opt,min_azimuth,max_azimuth,neighborhood_radius,custom_antenna_pattern,bs_eirp_dist,cell_aas_dist_data);
                            [cell_miti_list]=pre_sort_movelist_rev22_miti_clutter_mc_chunks_app(app,move_list_reliability,point_idx,sim_number,mc_size,radar_beamwidth,base_protection_pts,radar_threshold,mc_percentile,sim_array_list_bs,data_label1,reliability,norm_aas_zero_elevation_data,string_prop_model,array_mitigation,tf_opt,min_azimuth,max_azimuth,neighborhood_radius,custom_antenna_pattern,cell_aas_dist_data,cell_sim_data,sim_folder,parallel_flag,tf_man_azi_step,azimuth_step,move_list_margin,tf_server_status);
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        ...
                        % cell_miti_list_21d = (No secondary threshold)
                        %
                        %   5×5 cell array
                        %
                        %     {  0×9 double}    {0×0   double}    {[ 30]}    {0×0 double}    {0×0 double}
                        %     {  1×9 double}    {[         1]}    {[ 20]}    {[       1]}    {[       1]}
                        %     { 25×9 double}    {1×25  double}    {[ 10]}    {[       2]}    {[      26]}
                        %     {329×9 double}    {1×329 double}    {[  0]}    {[      27]}    {[     355]}
                        %     {285×9 double}    {1×285 double}    {[NaN]}    {[     356]}    {[     640]}

                        % cell_miti_list = (with secondary threshold)
                        %
                        %   5×5 cell array
                        %
                        %     {  0×9 double}    {0×0   double}    {[ 30]}    {0×0 double}    {0×0 double}
                        %     {  2×9 double}    {[       1 2]}    {[ 20]}    {[       1]}    {[       2]}
                        %     { 20×9 double}    {1×20  double}    {[ 10]}    {[       3]}    {[      22]}
                        %     { 34×9 double}    {1×34  double}    {[  0]}    {[      23]}    {[      56]}
                        %     {584×9 double}    {1×584 double}    {[NaN]}    {[      57]}    {[     640]}
                    ...
                    % % 'Do we want to do a final agg check at the end of the miti loop?'
                    % % 'Maybe after all the point_idx'



                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Find/Plot the Coordination Zones for each reliability
                    file_name_cell_bound_miti=strcat('cell_bound_Part3_',string_prop_model,'_',num2str(sim_number),'_',data_label1,'.mat');
