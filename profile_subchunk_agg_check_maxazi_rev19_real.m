function results = profile_subchunk_agg_check_maxazi_rev19_real(app,cell_aas_dist_data,array_bs_azi_data,radar_beamwidth,min_azimuth,max_azimuth,base_protection_pts,point_idx,on_list_bs,cell_sim_chunk_idx,rand_seed1,agg_check_reliability,on_full_Pr_dBm,clutter_loss,custom_antenna_pattern,sub_point_idx)
%PROFILE_SUBCHUNK_AGG_CHECK_MAXAZI_REV19_REAL
% Profile rev19 and compare clutter timing directly against golden rev11.
...
fprintf('AZI_CHUNK rev11: %d | rev19: %d\n',opts.AziChunkRev11,opts.AziChunkRev19);
...
% Measure rev19 profile on the same inputs.
[tbl,wall_runtime_s,top_total]=run_profile_once(@subchunk_agg_check_maxazi_rev19,opts.AziChunkRev19, ...
    opts.EnableDetailBuiltin,app,cell_aas_dist_data,array_bs_azi_data,radar_beamwidth, ...
    min_azimuth,max_azimuth,base_protection_pts,point_idx,on_list_bs,cell_sim_chunk_idx, ...
    rand_seed1,agg_check_reliability,on_full_Pr_dBm,clutter_loss,custom_antenna_pattern,sub_point_idx);
...
% Required reporting targets for rev19.
key=struct();
key.subchunk_agg_check_maxazi_rev19=summarize_rows(tbl,match_rows(tbl,'subchunk_agg_check_maxazi_rev19'),wall_runtime_s);
...
print_row('subchunk_agg_check_maxazi_rev19',key.subchunk_agg_check_maxazi_rev19);
...
fprintf('  subchunk total: rev11=%.6f s | rev19=%.6f s | speedup=%.3fx\n', ...
    baseline_wall_s,wall_runtime_s,baseline_wall_s/max(wall_runtime_s,eps));
fprintf('  clutter helper: rev11 rev3=%.6f s | rev19 rev5=%.6f s | drop=%.2f%%\n', ...
...
results.rev19_wall_runtime_s=wall_runtime_s;
results.speedup_rev11_over_rev19=baseline_wall_s/max(wall_runtime_s,eps);
results.top_by_total_rev11=baseline_top;
results.top_by_total_rev19=top_total;
results.summary_rev11=base;
results.summary_rev19=key;
...
results.full_profile_table_rev19=tbl;
...
if ~isfield(pinfo,'FunctionTable') || isempty(pinfo.FunctionTable)
    error('profile_subchunk_agg_check_maxazi_rev19_real:EmptyProfile', ...
...
if exist(fname,'file')~=2
    error(['profile_subchunk_agg_check_maxazi_rev19_real:' errid], ...
