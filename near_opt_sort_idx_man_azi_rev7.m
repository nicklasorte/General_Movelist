function [opt_sort_bs_idx,array_max_agg,array_uuid]=near_opt_sort_idx_man_azi_rev7(app,data_label1,point_idx,tf_calc_opt_sort,radar_beamwidth,single_search_dist,sim_array_list_bs,base_protection_pts,temp_pr_dbm,string_prop_model,custom_antenna_pattern,min_azimuth,max_azimuth,tf_man_azi_step,azimuth_step)
...
opt_sort_bs_idx_file_name=strcat(data_label1,'_',string_prop_model,'_opt_sort_bs_idx_agg_rev7_',num2str(point_idx),'_',num2str(single_search_dist),'km.mat')  %%%%%Slightly different name to not overwrite.
[var_exist_opt_sort]=persistent_var_exist_with_corruption(app,opt_sort_bs_idx_file_name);
array_max_agg_idx_file_name=strcat(data_label1,'_',string_prop_model,'_array_max_agg_rev7_',num2str(point_idx),'_',num2str(single_search_dist),'km.mat')
[var_exist_max_agg]=persistent_var_exist_with_corruption(app,array_max_agg_idx_file_name);
array_uuid_file_name=strcat(data_label1,'_',string_prop_model,'_uuid_rev7_',num2str(point_idx),'_',num2str(single_search_dist),'km.mat')
[var_exist_uuid]=persistent_var_exist_with_corruption(app,array_uuid_file_name);

if tf_calc_opt_sort==1
    var_exist_opt_sort=0;
end

if var_exist_opt_sort==2 && var_exist_max_agg==2 && var_exist_uuid==2
    %%%%%%%%%%%load
    retry_load=1;
    while(retry_load==1)
        try
            ...
            load(array_uuid_file_name,'array_uuid')
            retry_load=0;
        catch
            retry_load=1;
            pause(1)
    ...
    % 
    % 'This needs to get updated with the manual azimuths --> rev4 near_opt_sort_idx_rev5'
    %[array_sim_azimuth,num_sim_azi]=calc_sim_azimuths_rev3_360_azimuths_app(app,radar_beamwidth,min_azimuth,max_azimuth);
          [array_sim_azimuth,num_sim_azi]=calc_sim_azimuths_rev4_man_azi_app(app,radar_beamwidth,min_azimuth,max_azimuth,tf_man_azi_step,azimuth_step);
    ...
    % end
    % temp_Pr_watts_azi_og=temp_Pr_watts_azi;
    % toc;

    tic;
    [temp_Pr_watts_azi]=vector_pr_watts_azi_rev1(app,sim_array_list_bs,non_sort_bs_azimuth,temp_pr_dbm,custom_antenna_pattern,array_sim_azimuth);
    toc;
% % %     Elapsed time is 0.459645 seconds.
% % % Elapsed time is 0.006714 seconds.
% size(temp_Pr_watts_azi)
% size(temp_Pr_watts_azi_og)
% all(all(temp_Pr_watts_azi==temp_Pr_watts_azi_og))
    ...
    for i=1:1:num_tx
        ...
        new_agg_matrix = max(current_agg - candidate_contributions, 0);

        % % % Calculate max aggregate for each candidate removal scenario
        new_max_dbm_vec = max(pow2db(new_agg_matrix * 1000), [], 2);

        %%%%%%Fail loud: a candidate contribution should never exceed the aggregate.
        %%%%%%If clamping above did not catch it, pow2db of a negative goes complex.
        if ~isreal(new_max_dbm_vec)
            error('NearOptSortManAzi:ComplexAggregate',...
                'Complex dBm in near-optimal removal (point_idx=%d, single_search_dist=%d km, removal step %d of %d). Check temp_pr_dbm and off-axis gains for NaN/Inf.',...
                point_idx,single_search_dist,i,num_tx)
        end
    ...
    %%%'need to save the actual uuid of column 5 sim_array_list_bs'
    array_uuid=sim_array_list_bs(opt_sort_bs_idx,5);
    ...
    while(retry_save==1)
        try
            ...
            save(array_max_agg_idx_file_name,'array_max_agg')
             save(array_uuid_file_name,'array_uuid')
            retry_save=0;
        catch
            retry_save=1;
            pause(1)
...
%%%%%%Verify the greedy removal produced a non-increasing max-aggregate sequence.
%%%%%%Tolerance absorbs floating-point roundoff; a real increase is a logic error and stops the run.
array_max_agg_diff=diff(array_max_agg);
if max(array_max_agg_diff)>1e-9
    error('NearOptSortManAzi:NonMonotonicAggregate',...
        'Removal order is not near-optimal (point_idx=%d, single_search_dist=%d km): maximum aggregate increased by %.3g dB during greedy removal. Aggregate must be non-increasing as sectors are turned off.',...
        point_idx,single_search_dist,max(array_max_agg_diff))
end