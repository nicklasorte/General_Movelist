...
if isempty(sim_array_list_bs) || ~isnumeric(sim_array_list_bs)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20b_cust_ant_bsdist_app: sim_array_list_bs is empty or non-numeric'))
    pause;
end
if isempty(base_protection_pts) || ~isnumeric(base_protection_pts)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20b_cust_ant_bsdist_app: base_protection_pts is empty or non-numeric'))
    pause;
end
if isempty(reliability) || ~isnumeric(reliability)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20b_cust_ant_bsdist_app: reliability is empty or non-numeric'))
    pause;
end
if isempty(move_list_reliability) || ~isnumeric(move_list_reliability)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20b_cust_ant_bsdist_app: move_list_reliability is empty or non-numeric'))
    pause;
end
if isempty(custom_antenna_pattern) || ~isnumeric(custom_antenna_pattern)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20b_cust_ant_bsdist_app: custom_antenna_pattern is empty or non-numeric'))
    pause;
end
if ~isnumeric(mc_size) || ~isscalar(mc_size) || mc_size<1
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20b_cust_ant_bsdist_app: mc_size is invalid'))
    pause;
end
if ~isnumeric(point_idx) || ~isscalar(point_idx) || point_idx<1
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20b_cust_ant_bsdist_app: point_idx is invalid'))
    pause;
end
...
if isempty(move_sort_sim_array_list_bs)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20b_cust_ant_bsdist_app: move_sort_sim_array_list_bs is empty'))
    pause;
end
