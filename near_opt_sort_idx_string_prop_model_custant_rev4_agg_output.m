...
if isempty(sim_array_list_bs) || ~isnumeric(sim_array_list_bs)
    disp_progress(app,strcat('ERROR PAUSE: near_opt_sort_idx_string_prop_model_custant_rev4_agg_output: sim_array_list_bs is empty or non-numeric'))
    pause;
end
if isempty(base_protection_pts) || ~isnumeric(base_protection_pts)
    disp_progress(app,strcat('ERROR PAUSE: near_opt_sort_idx_string_prop_model_custant_rev4_agg_output: base_protection_pts is empty or non-numeric'))
    pause;
end
if isempty(temp_pr_dbm) || ~isnumeric(temp_pr_dbm)
    disp_progress(app,strcat('ERROR PAUSE: near_opt_sort_idx_string_prop_model_custant_rev4_agg_output: temp_pr_dbm is empty or non-numeric'))
    pause;
end
if isempty(custom_antenna_pattern) || ~isnumeric(custom_antenna_pattern)
    disp_progress(app,strcat('ERROR PAUSE: near_opt_sort_idx_string_prop_model_custant_rev4_agg_output: custom_antenna_pattern is empty or non-numeric'))
    pause;
end
if ~isnumeric(point_idx) || ~isscalar(point_idx) || point_idx<1
    disp_progress(app,strcat('ERROR PAUSE: near_opt_sort_idx_string_prop_model_custant_rev4_agg_output: point_idx is invalid'))
    pause;
end
...
if isempty(opt_sort_bs_idx)
    disp_progress(app,strcat('ERROR PAUSE: near_opt_sort_idx_string_prop_model_custant_rev4_agg_output: opt_sort_bs_idx is empty'))
    pause;
end
if isempty(array_max_agg)
    disp_progress(app,strcat('ERROR PAUSE: near_opt_sort_idx_string_prop_model_custant_rev4_agg_output: array_max_agg is empty'))
    pause;
end
