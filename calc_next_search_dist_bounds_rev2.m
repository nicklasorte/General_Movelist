function [next_search_dist,tf_search,temp_catb_dist_data,array_searched_dist,lower_bound_dist,upper_bound_dist]=calc_next_search_dist_bounds_rev2(app,all_data_stats_binary,radar_threshold,margin,maine_exception,tf_full_binary_search,min_binaray_spacing)
...
% % idx_zero=find(count_binary_bins==0)+1;

if tf_full_binary_search==1
    sort_idx_dist=sort(idx_nonzero,'descend'); %%%%%%Descend Sort, will search the higher end first
% %     if ~isempty(idx_zero)==1
% %         sort_zero_idx_dist=max(sort(idx_zero,'descend')); %%%%%Only need the max
% %     else
% %         sort_zero_idx_dist=1;
% %     end
end
...
%lower_bound_dist=array_searched_dist(sort_zero_idx_dist);
%upper_bound_dist=array_searched_dist(min(sort_idx_dist)+1);
...
if ~isnan(next_search_dist)==1
    ...
    lower_bound_dist_idx=find(array_searched_dist<next_search_dist,1,'last');
    lower_bound_dist=array_searched_dist(lower_bound_dist_idx);
    upper_bound_dist_idx=find(array_searched_dist>next_search_dist,1,'first');
    upper_bound_dist=array_searched_dist(upper_bound_dist_idx);
else
    tf_search=0;
    lower_bound_dist=NaN(1);
    upper_bound_dist=NaN(1);
end
