function part0_deployment_pts_folders_ue_rev9(app,sim_number,bs_eirp_reductions,rev_folder,tf_server_status,cell_sim_data,base_station_latlonheight,sim_radius_km,FreqMHz)

disp_TextArea_PastText(app,strcat('Part0: Line 3'))
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Function:
...
disp_TextArea_PastText(app,strcat('Part0: Line 10'))
...
disp_TextArea_PastText(app,strcat('Part0: Line 20'))
...
disp_TextArea_PastText(app,strcat('Part0: Line 34'))
if ~isempty(zero_idx)==1
    disp_TextArea_PastText(app,strcat('Part0: Line 36')) %%%%%%Error after this point
    temp_folder_names=create_folder_names(zero_idx)
    disp_TextArea_PastText(app,strcat('Part0: Line 37')) %%%%%%Error after this point
    num_folders=length(temp_folder_names)
    disp_TextArea_PastText(app,strcat('Part0: Line 39'))
    ...
    disp_TextArea_PastText(app,strcat('Part0: Line 50'))
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for folder_idx=1:1:num_folders
        disp_TextArea_PastText(app,strcat('Part0: Line 54'))
        ...
        [cell_status]=checkout_cell_status_rev1(app,checkout_filename,cell_status_filename,sim_folder,create_folder_names,tf_update_cell_status);
        %%%cell_status(:,2)=num2cell(0)
        disp_TextArea_PastText(app,strcat('Part0: Line 66'))
        ...
        disp_TextArea_PastText(app,strcat('Part0: Line 77'))
        if cell_status{temp_cell_idx,2}==0
            disp_TextArea_PastText(app,strcat('Part0: Line 79'))
            %%%%%%%%%%Calculate
            retry_cd=1;
            while(retry_cd==1)
                try
            ...
            folder_row_idx=find(matches(folder_names,sim_folder));
             disp_TextArea_PastText(app,strcat('Part0: Line 96'))
            ...
            data_label1=sim_folder;
             disp_TextArea_PastText(app,strcat('Part0: Line 120'))
            ...
            if var_exist==2
                ...
                while(retry_cd==1)
                 ...
                 disp_TextArea_PastText(app,strcat('Part0: Line 137'))
                ...
                toc;
                 disp_TextArea_PastText(app,strcat('Part0: Line 147'))
            else
                disp_TextArea_PastText(app,strcat('Part0: Line 149'))
                ...
                data_header=cell_sim_data(1,:)';
                 disp_TextArea_PastText(app,strcat('Part0: Line 157'))
                ...
                if var_exist_pp_pts~=2
                 ...
                 disp_TextArea_PastText(app,strcat('Part0: Line 179'))
                ...
                tf_ant_square_pattern=temp_single_cell_sim_data{tf_ant_square_idx}
                 ...
                 disp_TextArea_PastText(app,strcat('Part0: Line 222'))
                if tf_custom_ant_pat==1
                    ...
                    elseif matches(temp_str_ant_pattern,'itu_antenna_465')
                        col_ant_gain_idx=find(matches(data_header,'rx_ant_gain_mb'));
                        temp_ant_gain=temp_single_cell_sim_data{col_ant_gain_idx};
                        max_ant_gain_dBi=temp_ant_gain; %%%%%%%%dBi

                        [array_ant_gain]=ant_gain_itu_465_rev2(app,max_ant_gain_dBi)
                        max(array_ant_gain)
                      ...
                      % % % % % % % % % %%%%%%%%%%%This should probably be done on the server, since the
                        ...
                        if tf_ant_square_pattern==1
                            ...
                            for i=1:1:num_azi
                                ...
                                %%%Find the deltas in azimuth and elevation
                                %%%%
                                temp_azi=min(horzcat(360-temp_azi,temp_azi))
                                delta_azi=abs(gs_azimuth-temp_azi);
                 ...
                 disp_TextArea_PastText(app,strcat('Part0: Line 578'))
                ...
                while(retry_save==1)
                 ...
                 disp_TextArea_PastText(app,strcat('Part0: Line 598'))
                ...
                while(retry_save==1)
                 ...
                 disp_TextArea_PastText(app,strcat('Part0: Line 614'))
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                col_header_tf_ue_grid_idx=find(matches(data_header,'tf_ue_grid'));
                tf_ue_grid=temp_single_cell_sim_data{col_header_tf_ue_grid_idx}

                col_header_ue_grid_km_idx=find(matches(data_header,'ue_grid_km'));
                ue_grid_km=temp_single_cell_sim_data{col_header_ue_grid_km_idx}

                col_header_ue_height_m_idx=find(matches(data_header,'ue_height_m'));
                ue_height_m=temp_single_cell_sim_data{col_header_ue_height_m_idx}
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

  
                 disp_TextArea_PastText(app,strcat('Part0: Line 633'))
                %%%%%%%%Sim Bound
                ...
                if tf_ue_grid==0
                    ...
                    if isempty(bs_inside_idx)
                        disp_TextArea_PastText(app,strcat('Error: Empty bs_inside_idx: Part0 Grid Points:',data_label1))
                        'need to fix the empty base station error'
                        pause;
                    end
                ...
                elseif tf_ue_grid==1
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Generate Grid Points for UE since it is for the geofence
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%We can have another option later that just generates it around Base Stations
                    base_buffer=sim_bound;
                    grid_spacing=ue_grid_km;
                    bs_height=ue_height_m;
                    ...
                    % % % figure;
                    % % % hold on;
                    % % % plot(base_polygon(:,2),base_polygon(:,1),'sr')
                    % % % plot(sim_bound(:,2),sim_bound(:,1),'-g')
                    % % % plot(grid_points(:,2),grid_points(:,1),'ob')
                    % % % grid on;
                    % % % pause(0.1)

                    [num_tx,~]=size(grid_points);
                    sim_array_list_bs=grid_points;
                    sim_array_list_bs(:,9)=1;
                    %%%%%%%%Make everything Urban to be apart of the clutter
                    sim_array_list_bs(:,4)=bs_eirp_reductions;
                    sim_array_list_bs(:,5)=1:1:num_tx;
                    sim_array_list_bs(:,6)=3;  %%%%%%%%%EIRP number, just set to 3.
                    ...
                    disp_TextArea_PastText(app,strcat('Error: What else could this be: tf_ue_grid: Part0 Grid Points:',data_label1))
                    pause;
                end
                 disp_TextArea_PastText(app,strcat('Part0: Line 716'))
                ...
                while(retry_save==1)
                 ...
                 disp_TextArea_PastText(app,strcat('Part0: Line 770'))
                ...
                while(retry_save==1)
                 ...
                 disp_TextArea_PastText(app,strcat('Part0: Line 788'))
                ...
                while(retry_cd==1)
                 ...
                 disp_TextArea_PastText(app,strcat('Part0: Line 801'))
                %%%%%%%%Update the cell_status
                %%%%[cell_status]=update_generic_status_cell_rev1(app,folder_names,sim_folder,cell_status_filename);
                %%%[~]=update_generic_status_cell_rev1_debug(app,create_folder_names,sim_folder,cell_status_filename);
                %%%%%%%%Update the cell_status
                ...
                toc;
                server_status_rev2(app,tf_server_status)
                 disp_TextArea_PastText(app,strcat('Part0: Line 810'))
            end
             disp_TextArea_PastText(app,strcat('Part0: Line 813'))
        end
         disp_TextArea_PastText(app,strcat('Part0: Line 815'))
    ...
    try
     ...
     disp_TextArea_PastText(app,strcat('Part0: Line 824'))


    %%%%%%%%%%If we make it here, just mark all the cell_status as complete
    finish_cell_status_rev1(app,rev_folder,cell_status_filename)
     disp_TextArea_PastText(app,strcat('Part0: Line 829'))
end
server_status_rev2(app,tf_server_status)
 disp_TextArea_PastText(app,strcat('Part0: Line 832'))