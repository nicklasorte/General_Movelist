function [sub_array_agg_check_mc_dBm]=subchunk_agg_check_maxazi_rev20(app,cell_aas_dist_data,array_bs_azi_data,radar_beamwidth,min_azimuth,max_azimuth,base_protection_pts,point_idx,on_list_bs,cell_sim_chunk_idx,rand_seed1,agg_check_reliability,on_full_Pr_dBm,clutter_loss,custom_antenna_pattern,sub_point_idx,varargin)
%SUBCHUNK_AGG_CHECK_MAXAZI_REV20 Pr-focused optimization branch.
% rev20 is a rev19-semantic branch that swaps Pr helper (rev2 -> rev3)
% and is intended to be validated fail-closed against rev11.
...
% rev20: uses monte_carlo_Pr_dBm_rev3_app (vectorized, no nearestpoint_app).
% -------------------------------------------------------------------------
sort_monte_carlo_pr_dBm_all=NaN(num_bs,num_mc_idx);
for loop_idx=1:1:num_mc_idx
    pre_sort_monte_carlo_pr_dBm=monte_carlo_Pr_dBm_rev3_app(app,agg_check_reliability,on_full_Pr_dBm,rand_pr_all(:,loop_idx));
    [rand_norm_eirp]=monte_carlo_super_bs_eirp_dist_rev8(app,super_array_bs_eirp_dist,agg_check_reliability,rand_eirp_all(:,loop_idx));
    ...
    for azi_start=1:azi_chunk:num_sim_azi
        ...
        if DEBUG_CHECKS
            if any(isnan(sort_temp_mc_dBm),'all')
                error('subchunk_agg_check_maxazi_rev20:NaNTempDbm','NaN detected in sort_temp_mc_dBm');
            end
        end
        ...
        if DEBUG_CHECKS
            if any(isnan(binary_sort_mc_watts),'all')
                error('subchunk_agg_check_maxazi_rev20:NaNWatt','NaN detected in binary_sort_mc_watts');
            end
        end
