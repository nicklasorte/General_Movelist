...
% REV10 correctness-first orchestration cleanup.
% Strategy-1 scope: remove hard debug pauses from production flow while
% preserving rev9 output contract, chunk scheduling, and checkpoint/disk
% semantics. Assumes monte_carlo_Pr_dBm_rev3_app is already available
% downstream and does not re-implement that dependency in this revision.
...
if var_exist_agg_check==2 && var_exist_agg_dist==2
    ...
    if any(isnan(bs_azi_gain))
        ...
        error('agg_check_parfor_chunk_rev10_app:NaNGain','NaN detected in bs_azi_gain.');
    end

    if any(isnan(pathloss))
        find(isnan(pathloss));
        error('agg_check_parfor_chunk_rev10_app:NaNPathloss','NaN detected in pathloss.');
    end

    if any(isnan(on_full_Pr_dBm))
        on_full_Pr_dBm;
        error('agg_check_parfor_chunk_rev10_app:NaNPr','NaN detected in on_full_Pr_dBm.');
    end
    ...
    if isempty(on_full_Pr_dBm)
        ...
        for chunk_idx=1:num_chunks  %%%%%%%%%Parfor
            ...
            % Debug pause removed in rev10 to keep orchestration non-interactive.
        ...
        if all(isnan(array_agg_check_mc_dBm))
            error('agg_check_parfor_chunk_rev10_app:AllNaN','array_agg_check_mc_dBm is all NaN.');
        else
            %%%%%%%%Not NaN, can keep going
            %%%%%%%%%%%%%%Generate MC Iterations and Calculate Move List
            %array_agg_check_mc_dBm=NaN(mc_size,num_sim_azi);

            [mc_rows,num_azi]=size(array_agg_check_mc_dBm); %#ok<NASGU>
            if num_azi>1
                error('agg_check_parfor_chunk_rev10_app:UnsupportedAziColumns','num_azi>1 not yet supported in this orchestration path.');
            end
    ...
    % Hard debug stop removed in rev10; continue to cleanup and return.
