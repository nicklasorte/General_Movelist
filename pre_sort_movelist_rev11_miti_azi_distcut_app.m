function [uni_move_list_idx]=pre_sort_movelist_rev11_miti_azi_distcut_app(app,move_list_reliability,point_idx,sim_number,radar_beamwidth,base_protection_pts,min_ant_loss,radar_threshold,sim_array_list_bs,data_label1,reliability,norm_aas_zero_elevation_data,string_prop_model,tf_opt,min_azimuth,max_azimuth,single_search_dist,temp_miti,move_list_mc_size,move_list_mc_percentile)

%'Need to output the uni_bs_idx (column 5) since we are cutting due to the neighborhood'

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Move List Function with
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Azimuth Cut and Neighborhood
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Cut and Mitigation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

move_sort_file_name=strcat(string_prop_model,'_uni_move_list_idx_',num2str(min(move_list_reliability)),'_',num2str(max(move_list_reliability)),'_',num2str(point_idx),'_',num2str(sim_number),'_',num2str(move_list_mc_size),'_',num2str(move_list_mc_percentile),'_',num2str(temp_miti),'_',num2str(single_search_dist),'km.mat');
[var_exist_move_sort]=persistent_var_exist_with_corruption(app,move_sort_file_name);

%           sort_bs_idx_file_name=strcat(string_prop_model,'_sort_bs_idx_',num2str(min(move_list_reliability)),'_',num2str(max(move_list_reliability)),'_',num2str(point_idx),'_',num2str(sim_number),'_',num2str(move_list_mc_size),'_',num2str(move_list_mc_percentile),'_',num2str(temp_miti),'_',num2str(single_search_dist),'km.mat');
...
if var_exist_move_sort==2 %&& var_exist_sort_idx==2
    ...
    while(retry_load==1)
        try
            ...
            load(move_sort_file_name,'uni_move_list_idx')
            retry_load=0;
        catch
            retry_load=1;
            pause(1)
    ...
    [pathloss]=trim_pathloss_reliability_rev1(app,pathloss,move_list_reliability,reliability,string_prop_model);

    %%%%'Cut the base stations and pathloss to be only within the search distance'
    [sim_array_list_bs,keep_idx]=trim_base_stations_distance_rev1(app,base_protection_pts,point_idx,sim_array_list_bs,single_search_dist);

    %%%Check distance cut
    sim_pt=base_protection_pts(point_idx,:);
    bs_distance=deg2km(distance(sim_pt(1),sim_pt(2),sim_array_list_bs(:,1),sim_array_list_bs(:,2)));
    horzcat(max(bs_distance),single_search_dist)  %%%%%%%%This is right


    %%%%%%%%Cut the pathloss based on the neighborhood distance
    size(pathloss)
    ...
    % % % % % % % %%%%%'Update the EIPR into Column #4'(Doesn't seem to be used for this function.)
    ...
    temp_pr_dbm=sim_array_list_bs(:,4)-mid_pathloss_dB+bs_azi_gain-temp_miti;  %%%%%%%%%%%Non-Mitigation EIRP - Pathloss + BS Azi Gain = Power Received at Federal System (minus the mitigation applied)
    ...
    if tf_opt==0
        ...
        disp_progress(app,strcat('PAUSE: Inside Pre_sort_ML rev11: Need to double check optimal sort Logic Below'))
        pause;
    ...
    if any(isnan(sort_bs_idx))
        disp_progress(app,strcat('Error: PAUSE: Inside Pre_sort_ML: NaN Error on sort_bs_idx'))
        pause;
    end
    ...
    if any(isnan(bs_azi_gain))
        ...
        disp_progress(app,strcat('Error: PAUSE: Inside Pre_sort_ML: NaN error on bs_azi_gain'))
        pause;
    end

    if any(isnan(pathloss))
        find(isnan(pathloss))
        disp_progress(app,strcat('Error: PAUSE: Inside Pre_sort_ML: NaN error on pathloss'))
        pause;
    end

    if any(isnan(sim_array_list_bs(:,4)))
        find(isnan(sim_array_list_bs(:,4)))
        disp_progress(app,strcat('Error: PAUSE: Inside Pre_sort_ML: NaN error on sim_array_list_bs(:,4)'))
        pause;
    end
    ...
    if any(isnan(sort_full_Pr_dBm))
        ...
        disp_progress(app,strcat('Error: PAUSE: Inside Pre_sort_ML : NaN error on sort_full_Pr_dBm'))
        pause;
    end
    ...
    if isempty(sort_full_Pr_dBm)
        ...
        uni_move_list_idx=NaN(1,1);
        uni_move_list_idx=uni_move_list_idx(~isnan(uni_move_list_idx));
    else
        ...
        tempx=ceil(rand(1)*move_list_mc_size);
        tempy=ceil(rand(1)*move_list_mc_size);
        ...
        %%%Preallocate
        array_turn_off_size=NaN(move_list_mc_size,1);
        ...
        for mc_iter=1:1:move_list_mc_size
            ...
            if length(reliability)==1 %%%%%%%This assume 50%
                if ~all(sort_full_Pr_dBm==sort_monte_carlo_pr_dBm)
                    disp_progress(app,strcat('Error: Pause: Inside Pre_sort_ML :Error:Pr dBm Mismatch'))
                    pause;
                end
            end
            ...
            for azimuth_idx=1:1:num_sim_azi
                ...
                %%%%Maybe save this too, but only for the first mc-iteration, and only if there is one mc iteration.
                %if mc_iter==1
                ...
                if any(isnan(sort_temp_mc_dBm))  %%%%%%%%Check
                    disp_progress(app,strcat('Error: Pause: Inside Pre_sort_ML: NaN Error: temp_mc_dBm'))
                    pause;
                end
        ...
        turn_off_size=ceil(prctile(array_turn_off_size,move_list_mc_percentile)); %%%%This was interp between points, so need to ceiling it.
        ...
        if turn_off_size==0
            ...
            'Error: Inside Pre_sort_ML: Check the empty move_list_idx'
            %pause;
        else
            move_list_turn_off_idx=1:1:turn_off_size;
        end
    end
    ...
    uni_move_list_idx=unique(sim_array_list_bs(sort_bs_idx(move_list_turn_off_idx),5));
    ...
    if max(move_list_turn_off_idx)>num_tx_full
        ...
        disp_progress(app,strcat('Error: Pause: Inside Pre_sort_ML: Error Move list too large'))
        pause;
        ...
        while(retry_save==1)
            try
                save(move_sort_file_name,'uni_move_list_idx') %%%%%%These are the index of the sort_sim_array_list_bs
                retry_save=0;
            catch
                retry_save=1;
                pause(1)
