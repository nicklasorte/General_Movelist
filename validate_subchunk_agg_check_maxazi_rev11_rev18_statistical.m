...
%VALIDATE_SUBCHUNK_AGG_CHECK_MAXAZI_REV11_REV18_STATISTICAL
% Fail-closed statistical validation of rev18 against golden rev11 only.
...
runtime18=zeros(opts.NumTimingRuns,1);
out11=[];
out18=[];
for run_idx=1:opts.NumTimingRuns
    ...
    t=tic;
    out18=subchunk_agg_check_maxazi_rev18(app,cell_aas_dist_data,array_bs_azi_data, ...
        radar_beamwidth,min_azimuth,max_azimuth,base_protection_pts,point_idx,on_list_bs, ...
        cell_sim_chunk_idx,rand_seed1,agg_check_reliability,on_full_Pr_dBm,clutter_loss, ...
        custom_antenna_pattern,sub_point_idx,opts.AziChunkRev18);
    runtime18(run_idx)=toc(t);
end

x11=out11(:);
x18=out18(:);
mask=isfinite(x11) & isfinite(x18);
x11=x11(mask);
x18=x18(mask);
if isempty(x11)
    error('validate_subchunk_agg_check_maxazi_rev11_rev18_statistical:NoFiniteSamples', ...
...
for k=1:numel(metric_names)
    ...
    drift.(m)=struct('rev11',v11,'rev18',v18,'abs_diff',abs_diff, ...
...
fprintf('  rev18 runs: [%s]\n',fmt_vec(runtime18));
fprintf('  rev11 mean: %.6f\n',mean(runtime11));
fprintf('  rev18 mean: %.6f\n',mean(runtime18));
fprintf('  speedup (rev11/rev18): %.3fx\n',mean(runtime11)/max(mean(runtime18),eps));

fprintf('\nMetric drift summary:\n');
for k=1:numel(metric_names)
    m=metric_names{k};
    d=drift.(m);
    fprintf('  %-7s | rev11=%10.4f | rev18=%10.4f | abs=%8.4f | allow=%8.4f | %s\n', ...
        m,d.rev11,d.rev18,d.abs_diff,d.allowed_abs,passfail(d.pass));
end
...
if overall_pass
    ...
    fprintf('\nPASS / FAIL: FAIL\n');
    error('validate_subchunk_agg_check_maxazi_rev11_rev18_statistical:DriftExceeded', ...
        'Fail-closed: rev18 drift exceeded rev11 thresholds.');
end
...
results.runtime_rev18_s=runtime18;
results.runtime_mean_rev11_s=mean(runtime11);
results.runtime_mean_rev18_s=mean(runtime18);
results.speedup_rev11_over_rev18=mean(runtime11)/max(mean(runtime18),eps);
results.summary_rev11=s11;
results.summary_rev18=s18;
...
if exist(fname,'file')~=2
    error(['validate_subchunk_agg_check_maxazi_rev11_rev18_statistical:' errid], ...
