function results = validate_subchunk_agg_check_maxazi_rev11_rev14_statistical(app,cell_aas_dist_data,array_bs_azi_data,radar_beamwidth,min_azimuth,max_azimuth,base_protection_pts,point_idx,on_list_bs,cell_sim_chunk_idx,rand_seed1,agg_check_reliability,on_full_Pr_dBm,clutter_loss,custom_antenna_pattern,sub_point_idx)
%VALIDATE_SUBCHUNK_AGG_CHECK_MAXAZI_REV11_REV14_STATISTICAL
% Run rev11 and rev14 on identical inputs, compare runtime and distribution tails.
% Fail-closed policy: throw error if configured drift thresholds are exceeded.

must_exist('subchunk_agg_check_maxazi_rev11','MissingRev11');
must_exist('subchunk_agg_check_maxazi_rev14','MissingRev14');

opts=struct();
opts.AziChunkRev11=128;
opts.AziChunkRev14=128;
opts.NumTimingRuns=3;
opts.CoreAbsDrift_dB=0.50;      % mean/std/min/max/median/p90
opts.CoreRelDrift=0.05;
opts.TailAbsDrift_dB=0.25;      % p95/p99/p99.9
opts.TailRelDrift=0.02;
opts.RequireP999Samples=1000;

fprintf('\n=== VALIDATION: REV11 vs REV14 (STATISTICAL) ===\n');
fprintf('Configured thresholds:\n');
fprintf('  core metrics: max(abs, rel)=max(%.3f dB, %.3f*|rev11|)\n',opts.CoreAbsDrift_dB,opts.CoreRelDrift);
fprintf('  tail metrics: max(abs, rel)=max(%.3f dB, %.3f*|rev11|)\n',opts.TailAbsDrift_dB,opts.TailRelDrift);

runtime11=zeros(opts.NumTimingRuns,1);
runtime14=zeros(opts.NumTimingRuns,1);
out11=[];
out14=[];
for run_idx=1:opts.NumTimingRuns
    t=tic;
    out11=subchunk_agg_check_maxazi_rev11(app,cell_aas_dist_data,array_bs_azi_data, ...
        radar_beamwidth,min_azimuth,max_azimuth,base_protection_pts,point_idx,on_list_bs, ...
        cell_sim_chunk_idx,rand_seed1,agg_check_reliability,on_full_Pr_dBm,clutter_loss, ...
        custom_antenna_pattern,sub_point_idx,opts.AziChunkRev11);
    runtime11(run_idx)=toc(t);

    t=tic;
    out14=subchunk_agg_check_maxazi_rev14(app,cell_aas_dist_data,array_bs_azi_data, ...
        radar_beamwidth,min_azimuth,max_azimuth,base_protection_pts,point_idx,on_list_bs, ...
        cell_sim_chunk_idx,rand_seed1,agg_check_reliability,on_full_Pr_dBm,clutter_loss, ...
        custom_antenna_pattern,sub_point_idx,opts.AziChunkRev14);
    runtime14(run_idx)=toc(t);
end

x11=out11(:);
x14=out14(:);
mask=isfinite(x11) & isfinite(x14);
x11=x11(mask);
x14=x14(mask);
if isempty(x11)
    error('validate_subchunk_agg_check_maxazi_rev11_rev14_statistical:NoFiniteSamples', ...
        'No finite paired samples available for comparison.');
end

metric_names={'mean','std','min','max','median','p90','p95','p99'};
qvals=[0.90 0.95 0.99];

s11=summary_stats(x11,qvals);
s14=summary_stats(x14,qvals);
if numel(x11)>=opts.RequireP999Samples
    s11.p99_9=quantile(x11,0.999);
    s14.p99_9=quantile(x14,0.999);
    metric_names{end+1}='p99_9'; %#ok<AGROW>
end

tail_metrics={'p95','p99','p99_9'};
drift=struct();
pass_vec=true(size(metric_names));
for k=1:numel(metric_names)
    m=metric_names{k};
    v11=s11.(m);
    v14=s14.(m);
    abs_diff=abs(v14-v11);
    rel_diff=abs_diff/max(abs(v11),eps);

    is_tail=any(strcmp(m,tail_metrics));
    if is_tail
        allowed=max(opts.TailAbsDrift_dB,opts.TailRelDrift*max(abs(v11),1));
    else
        allowed=max(opts.CoreAbsDrift_dB,opts.CoreRelDrift*max(abs(v11),1));
    end

    pass_here=abs_diff<=allowed;
    pass_vec(k)=pass_here;
    drift.(m)=struct('rev11',v11,'rev14',v14,'abs_diff',abs_diff, ...
        'rel_diff',rel_diff,'allowed_abs',allowed,'is_tail',is_tail,'pass',pass_here);
end

overall_pass=all(pass_vec);

fprintf('\nRuntime summary (seconds):\n');
fprintf('  rev11 runs: [%s]\n',fmt_vec(runtime11));
fprintf('  rev14 runs: [%s]\n',fmt_vec(runtime14));
fprintf('  rev11 mean: %.6f\n',mean(runtime11));
fprintf('  rev14 mean: %.6f\n',mean(runtime14));
fprintf('  speedup (rev11/rev14): %.3fx\n',mean(runtime11)/mean(runtime14));

fprintf('\nMetric drift summary:\n');
for k=1:numel(metric_names)
    m=metric_names{k};
    d=drift.(m);
    fprintf('  %-7s | rev11=%10.4f | rev14=%10.4f | abs=%8.4f | allow=%8.4f | %s\n', ...
        m,d.rev11,d.rev14,d.abs_diff,d.allowed_abs,passfail(d.pass));
end

fprintf('\nUpper-tail emphasis:\n');
for k=1:numel(tail_metrics)
    m=tail_metrics{k};
    if isfield(drift,m)
        d=drift.(m);
        fprintf('  %-7s | abs=%8.4f | allow=%8.4f | %s\n',m,d.abs_diff,d.allowed_abs,passfail(d.pass));
    end
end

if overall_pass
    fprintf('\nPASS: rev14 statistical behavior is within configured thresholds.\n');
else
    fprintf('\nFAIL: rev14 drift exceeded configured thresholds (fail-closed).\n');
    error('validate_subchunk_agg_check_maxazi_rev11_rev14_statistical:DriftExceeded', ...
        'Fail-closed: one or more statistical metrics exceeded thresholds.');
end

results=struct();
results.options=opts;
results.n_samples=numel(x11);
results.runtime_rev11_s=runtime11;
results.runtime_rev14_s=runtime14;
results.runtime_mean_rev11_s=mean(runtime11);
results.runtime_mean_rev14_s=mean(runtime14);
results.speedup_rev11_over_rev14=mean(runtime11)/mean(runtime14);
results.summary_rev11=s11;
results.summary_rev14=s14;
results.metrics=metric_names;
results.drift=drift;
results.pass=overall_pass;

end

function s=summary_stats(x,qvals)
s=struct();
s.mean=mean(x,'omitnan');
s.std=std(x,0,'omitnan');
s.min=min(x,[],'omitnan');
s.max=max(x,[],'omitnan');
s.median=median(x,'omitnan');
q=quantile(x,qvals);
s.p90=q(1);
s.p95=q(2);
s.p99=q(3);
end

function must_exist(fname,errid)
if exist(fname,'file')~=2
    error(['validate_subchunk_agg_check_maxazi_rev11_rev14_statistical:' errid], ...
        '%s.m was not found on MATLAB path.',fname);
end
end

function txt=fmt_vec(v)
txt=sprintf('%.6f ',v);
txt=strtrim(txt);
end

function txt=passfail(tf)
if tf
    txt='PASS';
else
    txt='FAIL';
end
end
