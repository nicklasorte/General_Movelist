    ...
    for folder_idx=1:1:num_folders
        ...
        if cell_status{temp_cell_idx,2}==0
            ...
            if var_exist==2
                    ...
                    if any(temp_data_dist==single_search_dist)==1
                        ...
                        if var_exist_single_scrap_data==2
                            ...
                            if file_union_move_exist==2
                                ...
                                for point_idx=1:1:num_ppts  %%%%%%%%This can be parfor
                                    ...
                                    if tf_test==1
                                        ...
                                        if ~isnan(move_sort_sim_array_list_bs(1,1)) && ~isnan(movelist_test(1,1))
                                            [ml_size1,~]=size(move_sort_sim_array_list_bs)
                                            [ml_size2,~]=size(movelist_test)
                                            
                                            if ml_size1~=ml_size2
                                                if abs(ml_size1-ml_size2)>1
                                                    'More than 1 ML size diff'
                                                    %pause;
                                                end
                                            else %%%%%Same size, check for identical
                                                if all(all(movelist_test==move_sort_sim_array_list_bs))==1
                                                    'The same move list'
                                                    %pause;
                                                else
                                                    'Error on different movelist '
                                                    %pause;
                                                end
                                            end
                                            %%%%%%%Secondary is already in the Move List

                                        elseif ~isnan(move_sort_sim_array_list_bs(1,1)) && isnan(movelist_test(1,1))
                                            ~isnan(move_sort_sim_array_list_bs(1,1))
                                            isnan(movelist_test(1,1))
                                            'Error: One ML is NaN and the other isnt'
                                            'movelist_test and move_sort_sim_array_list_bs'
                                            movelist_test
                                            move_sort_sim_array_list_bs
                                            %pause;
                                        elseif isnan(move_sort_sim_array_list_bs(1,1)) && ~isnan(movelist_test(1,1))
                                            isnan(move_sort_sim_array_list_bs(1,1))
                                            ~isnan(movelist_test(1,1))
                            ...
                            for point_idx=1:1:num_ppts  %%%%%%%%This can be parfor
                                ...
                                if tf_test==1
                                    test_label='TEST1'
                                    %[agg_check_95_test,array_agg_check_test]=agg_check_parfor_chunk_rev7_app_TEST(app,agg_check_reliability,point_idx,sim_number,mc_size,radar_beamwidth,base_protection_pts,mc_percentile,on_list_bs,data_label1,reliability,norm_aas_zero_elevation_data,string_prop_model,single_search_dist,off_idx,min_azimuth,max_azimuth,custom_antenna_pattern,cell_aas_dist_data,cell_sim_data,sim_folder,parallel_flag,test_label);
                                    %%%%%%%%Rev 6 was the pre-mc-chunk
                                    [agg_check_95_test,array_agg_check_test]=agg_check_rev6_clutter_app_TEST(app,agg_check_reliability,point_idx,sim_number,mc_size,radar_beamwidth,base_protection_pts,mc_percentile,on_list_bs,data_label1,reliability,norm_aas_zero_elevation_data,string_prop_model,single_search_dist,off_idx,min_azimuth,max_azimuth,custom_antenna_pattern,cell_aas_dist_data,cell_sim_data,sim_folder,test_label);

                                    horzcat(array_agg_check_95,agg_check_95_test)
                                    horzcat(sort(array_agg_check_mc_dBm),sort(array_agg_check_test))
                                    
                                    if all(array_agg_check_95==agg_check_95_test)==1 && all(array_agg_check_mc_dBm==array_agg_check_test)==1
                                        'The same agg check, very rare since the monte carlos randomization wont match up'
                                    else
                                        if abs(array_agg_check_95-agg_check_95_test)>1
                                            horzcat(array_agg_check_95,agg_check_95_test,abs(array_agg_check_95-agg_check_95_test))
                                            'Error: More than 1dB error on First Threshold'
                                            'This Matters if the number of mc-iterations is high, does not matter if mc-iterations is low'
                                            pause
                                        elseif max(abs(sort(array_agg_check_mc_dBm)-sort(array_agg_check_test)))>2
                                            abs(sort(array_agg_check_mc_dBm)-sort(array_agg_check_test))
                                            max(abs(sort(array_agg_check_mc_dBm)-sort(array_agg_check_test)))
                                            'This Matters if the number of mc-iterations is high, does not matter if mc-iterations is low'
                                            'More than 2dB Error somewhere on the array for the agg check '
                                            %pause;
                                        end
                                    end
                                    if tf_second_data==1
                                        agg_check_second=prctile(sort(array_agg_check_mc_dBm),mc_per2);
                                        agg_check_second_test=prctile(sort(array_agg_check_test),mc_per2);

                                        horzcat(agg_check_second,agg_check_second_test)
                                        if abs(agg_check_second-agg_check_second_test)>1
                                            horzcat(agg_check_second,agg_check_second_test,abs(agg_check_second-agg_check_second_test))
                                            'Error: More than 1dB error on Second Threshold'
                                            'This Matters if the number of mc-iterations is high, does not matter if mc-iterations is low'
                                            %pause
                                        end
                                    end
                            ...
                            if tf_second_data==1
                                temp_max_second_agg=max(second_scrap_data(:,1));
                            else
                                temp_max_second_agg=NaN(1,1);
                            end
                ...
                neighborhood_radius=catb_neighborhood_radius;
                retry_save=1;
                while(retry_save==1)
                    try
                ...
                %%%%'For this table, update to I/N instead of DPA-threshold numbers'
                ...
                %%%%%%%%%%%%%%%%
                if tf_second_data==1
                    second_temp_array=horzcat(all_secondary_stats_binary{:})
                    [num_row,num_col]=size(second_temp_array)

                    %%%%%Need the secondary, if they are there
                    in2_idx=find(matches(data_header,'second_in_ratio'));
                    per2_idx=find(matches(data_header,'second_mc_percentile'));
                    second_threshold_idx=find(matches(data_header,'dpa_second_threshold'));
                    second_radar_threshold=cell_sim_data{row_folder_idx,second_threshold_idx};
                    ...
                    while(retry_save==1)
                        try
                            writetable(table_second_stats,strcat('Stats_Neighborhood_Secondardy_',data_label1,'_',string_prop_model,'_Rev',num2str(sim_number),'.xlsx'));
                            pause(0.1);
                            retry_save=0;
                        catch
                            retry_save=1;
                ...
                %if all(array_mitigation==0)  %%%%%%%%Will do the pop impact based on part 2 results.
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    cell_miti_union_square=vertcat(cell_neigh,cell_miti_union);
                    [num_miti_rows,~]=size(cell_miti_union_square);
                    ...
                    census_pop=new_full_census_2020(:,4);
                    census_geoid=new_full_census_2020(:,1);
