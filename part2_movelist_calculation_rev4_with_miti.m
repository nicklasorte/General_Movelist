function part2_movelist_calculation_rev4_with_miti(app,folder_names,parallel_flag,rev_folder,workers,move_list_reliability,sim_number,move_list_mc_size,move_list_mc_percentile,reliability,norm_aas_zero_elevation_data,string_prop_model,mitigation_dB,tf_server_status,tf_opt)
    ...
    for folder_idx=1:1:num_folders
        ...
        if cell_status{temp_cell_idx,2}==0
            ...
            if var_exist==2
                ...
                [move_list_reliability]=trim_ml_rel_tirem_rev1(app,string_prop_model,move_list_reliability);
                ...
                file_name_array_miti_distance=strcat(string_prop_model,'_',data_label1,'_array_miti_distance_',num2str(min(move_list_reliability)),'_',num2str(max(move_list_reliability)),'_',num2str(sim_number),'_',num2str(move_list_mc_size),'_',num2str(single_search_dist),'km.mat');
                        ...
                        if file_union_move_exist==0 %%%The File Does not exist, we will calculate it
                            ...
                            if parallel_flag==1
                                parfor point_idx=1:num_ppts  %%%%Change to parfor
                                    pre_sort_movelist_rev11_miti_azi_distcut_app(app,move_list_reliability,point_idx,sim_number,radar_beamwidth,base_protection_pts,min_ant_loss,radar_threshold,sim_array_list_bs,data_label1,reliability,norm_aas_zero_elevation_data,string_prop_model,tf_opt,min_azimuth,max_azimuth,single_search_dist,temp_miti,move_list_mc_size,move_list_mc_percentile);
                            ...
                            for point_idx=1:1:num_ppts  %%%%%%%%This can be parfor
                                point_idx
                                [uni_move_list_idx]=pre_sort_movelist_rev11_miti_azi_distcut_app(app,move_list_reliability,point_idx,sim_number,radar_beamwidth,base_protection_pts,min_ant_loss,radar_threshold,sim_array_list_bs,data_label1,reliability,norm_aas_zero_elevation_data,string_prop_model,tf_opt,min_azimuth,max_azimuth,single_search_dist,temp_miti,move_list_mc_size,move_list_mc_percentile);
                                cell_move_list_idx{point_idx}=uni_move_list_idx;
                            ...
                            [off_idx]=nearestpoint_app(app,array_uni_nick_id_move_list_idx,sim_array_list_bs(:,5));
                            if ~all(array_uni_nick_id_move_list_idx==off_idx)
                                'Might need to check this'
                                pause;
                            end
                            
                            union_turn_off_list_data=sim_array_list_bs(off_idx,:);
