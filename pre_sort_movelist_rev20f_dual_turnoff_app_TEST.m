...
move_sort_file_name=strcat(test_label,'_',string_prop_model,'_move_sort_sim_array_list_bs_',num2str(min(move_list_reliability)),'_',num2str(max(move_list_reliability)),'_',num2str(point_idx),'_',num2str(sim_number),'_',num2str(mc_size),'_',num2str(single_search_dist),'km.mat');
    ...
    if tf_full_turnoff==1
        ...
        if isempty(sort_full_Pr_dBm)
            ...
            for mc_iter=1:1:mc_size
                ...
                secondary_azimuth_turn_off_size=NaN(num_sim_azi,1);
                for azimuth_idx=1:1:num_sim_azi
                    ...
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Binary Search
                    %%%[old_mid]=pre_sort_binary_movelist_rev2_app(app,radar_threshold,binary_sort_mc_watts);
                    [mid]=pre_sort_binary_movelist_rev2_app(app,radar_threshold-move_list_margin,binary_sort_mc_watts);
                    azimuth_turn_off_size(azimuth_idx)=mid;

                    if tf_second_data==1
                        %%%%Rerun binary
                        %%%'start here with another binary with the radar2threshold'
                        [mid_second]=pre_sort_binary_movelist_rev2_app(app,radar2threshold-move_list_margin,binary_sort_mc_watts);
                        secondary_azimuth_turn_off_size(azimuth_idx)=mid_second;
                    end
                end
                array_turn_off_size(mc_iter)=max(azimuth_turn_off_size); %%%%%%%%%%%max across all azimuths for a single MC iteration

                if tf_second_data==1
                    secondary_turn_off_size(mc_iter)=max(secondary_azimuth_turn_off_size); %%%%%%%%%%%max across all azimuths for a single MC iteration
                end
            end
            ...
            grid on;
            pause(0.1)
            filename1=strcat(test_label,'_','MoveList_',num2str(point_idx),'_',num2str(single_search_dist),'km.png');
            retry_save=1;
            while(retry_save==1)
                try
        ...
        move_array_turn_off_size_file_name=strcat(test_label,'_',string_prop_model,'_array_turn_off_size_',num2str(min(move_list_reliability)),'_',num2str(max(move_list_reliability)),'_',num2str(point_idx),'_',num2str(sim_number),'_',num2str(mc_size),'_',num2str(single_search_dist),'km.mat');
