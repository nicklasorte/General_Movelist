function [move_sort_sim_array_list_bs]=pre_sort_movelist_rev20c_cust_ant_superbsdist_app(app,move_list_reliability,point_idx,sim_number,mc_size,radar_beamwidth,base_protection_pts,radar_threshold,mc_percentile,sim_array_list_bs,data_label1,reliability,norm_aas_zero_elevation_data,string_prop_model,single_search_dist,tf_opt,min_azimuth,max_azimuth,custom_antenna_pattern,cell_aas_dist_data)


%%%%%Input validation
if isempty(sim_array_list_bs) || ~isnumeric(sim_array_list_bs)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20c_cust_ant_superbsdist_app: sim_array_list_bs is empty or non-numeric'))
    pause;
end
if isempty(base_protection_pts) || ~isnumeric(base_protection_pts)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20c_cust_ant_superbsdist_app: base_protection_pts is empty or non-numeric'))
    pause;
end
if isempty(reliability) || ~isnumeric(reliability)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20c_cust_ant_superbsdist_app: reliability is empty or non-numeric'))
    pause;
end
if isempty(move_list_reliability) || ~isnumeric(move_list_reliability)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20c_cust_ant_superbsdist_app: move_list_reliability is empty or non-numeric'))
    pause;
end
if isempty(custom_antenna_pattern) || ~isnumeric(custom_antenna_pattern)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20c_cust_ant_superbsdist_app: custom_antenna_pattern is empty or non-numeric'))
    pause;
end
if ~isnumeric(mc_size) || ~isscalar(mc_size) || mc_size<1
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20c_cust_ant_superbsdist_app: mc_size is invalid'))
    pause;
end
if ~isnumeric(point_idx) || ~isscalar(point_idx) || point_idx<1
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20c_cust_ant_superbsdist_app: point_idx is invalid'))
    pause;
end
...
if var_exist_move_sort==2
    ...
    % if all(norm_aas_zero_elevation_data(:,2)==array_50_aas_dist)~=1
    %     'norm_aas_zero_elevation_data and array_50_aas_dist do not match'
    %     pause;
    % end

    % %%%%'Check to see if  aas_dist_azimuth and bs_eirp_dist are the same at 0 azimuth, it's all off by 0.5, which is based on how we normalized it.'
    % zero_azi_idx=find(aas_dist_azimuth==0)
    % horzcat(bs_eirp_dist,array_aas_dist_data(zero_azi_idx,:)')
    % bs_eirp_dist(:,2)-array_aas_dist_data(zero_azi_idx,:)'
...
if isempty(move_sort_sim_array_list_bs)
    disp_progress(app,strcat('ERROR PAUSE: pre_sort_movelist_rev20c_cust_ant_superbsdist_app: move_sort_sim_array_list_bs is empty'))
    pause;
end
